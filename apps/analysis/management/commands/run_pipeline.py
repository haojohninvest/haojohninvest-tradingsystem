...
class Command(BaseCommand):
    help = '批次執行管線: calc_indicators → calc_market_breadth → calc_divergence → stock_pick_scanner'

    STEP_CHOICES = ['indicators', 'breadth', 'divergence', 'scanner', 'all']
    ...
    def handle(self, *args, **options):
        ...
        steps_to_run = self.STEP_CHOICES[:-1] if step == 'all' else [step]
